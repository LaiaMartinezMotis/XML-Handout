...
bool j1Render::Load(pugi::xml_node& renderer)  {

	camera.x = renderer.child("camera").attribute("x").as_int();
	//render(hijo de render)->camera(atributo de camera)->x

	camera.y = renderer.child("camera").attribute("y").as_int();
	//render(hijo de render)->camera(atributo de camera)->y
	return true;
}
...
bool j1Render::Save(pugi::xml_node& renderer) {
	
	renderer.child("camera").attribute("x").set_value(camera.x);
	renderer.child("camera").attribute("y").set_value(camera.y);;
