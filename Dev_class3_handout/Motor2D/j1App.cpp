...
bool j1App::Load()  {
	bool ret = LoadSavedfile();
	LOG("HOLAAAAAAA: %i", ret);
	if (ret == true)
	{
		p2List_item<j1Module*>* item;
		item = modules.start;
...
bool j1App::LoadSavedfile()
{
	bool ret = true;

	pugi::xml_parse_result result = save_file.load_file("save.xml");
