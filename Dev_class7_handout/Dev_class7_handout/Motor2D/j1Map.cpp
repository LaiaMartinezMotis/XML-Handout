...
void j1Map::Draw()
{
	...
	p2List_item<MapLayer*>* mapa = data.layers.start;

	while (mapa != nullptr) {
		// TODO 4: Make sure we draw all the layers and not just the first one
		MapLayer* layer = mapa->data;
...
TileSet* j1Map::GetTilesetFromTileId(int id) const
{
	...
	p2List_item<TileSet*>* tileid= data.tilesets.start;

	if (id >= tileid->next->data->firstgid)
	{
		tileid = tileid->next;
	}
	/*if (id <= tileid->next->data->firstgid)
